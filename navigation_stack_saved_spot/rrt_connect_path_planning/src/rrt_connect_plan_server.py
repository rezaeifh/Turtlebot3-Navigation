...
import rospy
from rrt_connect_path_planning.srv import PathPlanningPlugin, PathPlanningPluginResponse
from geometry_msgs.msg import Twist
from rrt_connect import Planning
...
def make_plan(req):
  ...
  # Calculate a path using RRT-Connect
  
  rrt_con= Planning(initial_position, target_position, width, height, map)
  
  path = rrt_con.rrt_connect()

  if not path:
    rospy.logwarn("No path returned by RRT-Connect")
    return_path = []
  else:
    # print time statistics
    execution_time = rospy.Time.now() - start_time
    print("\n")
    rospy.loginfo('+++++++++++ RRT-Connect execution metrics ++++++++++')
    rospy.loginfo('Total execution time: %s seconds', str(execution_time.to_sec()))
    rospy.loginfo('++++++++++++++++++++++++++++++++++++++++++++')
    print("\n")
    rospy.loginfo('RRT-Connect: Path sent to navigation stack')
...
if __name__ == '__main__':
  rospy.init_node('rrt_connect_path_planning_service_server', log_level=rospy.INFO, anonymous=False)
